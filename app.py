...
st.set_page_config(page_title='Iris Project - Gauri')

#Add a title in browser body
st.title('Iris End to End Project - Gauri Gund')

# Take sepal length , sepal width, petal length, petal widh
sep_len = st.number_input('Sepal Length : ', min_value = 0.00, step=0.01)
sep_width = st.number_input('Sepal Width : ', min_value = 0.00, step=0.01)
pet_len = st.number_input('Petal Length : ', min_value = 0.00, step=0.01)
pet_width = st.number_input('Petal Width : ', min_value = 0.00, step=0.01)

# Add a button to predict species
submit = st.button('Predict')

# Write  a function predict species along with probability
def predict_species(pre_path, model_path):
    # Get the inputs in dataframe format
    xnew = pd.DataFrame([sep_len, sep_width, pet_len, pet_width]).T
    xnew.columns = ['sepal_length', 'sepal_width', 'petal_length', 'petal_width']
    # Load the preprocessor with pickel
    with open(pre_path, 'rb') as file1:
        pre = pickle.load(file1) 

    # transform xnew
        xnew_pre = pre.transform(xnew)
        # Load the model with pickel
